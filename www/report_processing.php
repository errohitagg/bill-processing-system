...
<td width="100%">
	...
	<?php
		...
		$sql .= " bill_process.file_id=file.file_id AND bill.supplier_id=supplier.supplier_id AND bill.budget_id=budget.budget_id) WHERE ";
		$sql .= " bill_process.status='Processing' ORDER BY bill_process.process_id DESC LIMIT $start, 10";
		$result = mysqli_query($conn,$sql) or die("Couldn't execute query feedback_4");
		$rows=mysqli_num_rows($result);
		echo "<font size = 3 color = blue ><u><b>List of processing bills</u></b></font>";
		echo "<table width='90%'>";
			if($rows==0)
			{
			...
			else
			{
				...
				while ($myrow = mysqli_fetch_array($result))
				{
					...
					echo $tr_html.$myrow["bill_no"].$td_html.$myrow["name"].$td_html.$myrow["file_no"].$td_html.$myrow["budget_head"].
						 $td_html.change2dmy($myrow["date"]).$td_html.$myrow["amount"].$td_html."<a target='_blank' href=\"bill_slip.php?id=".
						 $myrow["process_id"]."\">View</a> ";
					echo "</u></font>";
				}
			}
